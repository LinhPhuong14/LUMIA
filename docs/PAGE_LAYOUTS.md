...
> Phiên bản: 2026-06-11 · Nguồn: `src/app/**/page.tsx` và component con.
...
Dùng cho `/`. Không có `SiteHeader` hay `DashboardShell`.

```
┌─────────────────────────────────────────────────────────────┐
│  FloatingNavbar (fixed, anchor in-page)                     │
├─────────────────────────────────────────────────────────────┤
│  Full-width sections (mỗi section dùng .landing-frame)      │
├─────────────────────────────────────────────────────────────┤
│  FooterSection                                              │
└─────────────────────────────────────────────────────────────┘
```
...
### 1.2 Shell B — Marketing

Dùng cho catalog, checkout, admin hub.

```
┌─────────────────────────────────────────────────────────────┐
│  SiteHeader (sticky)                                        │
│  Logo · nav links · Đăng nhập / Tạo tài khoản / user name   │
├─────────────────────────────────────────────────────────────┤
│  main (.shell hoặc max-w-[1280px])                          │
├─────────────────────────────────────────────────────────────┤
│  SiteFooter (tuỳ trang)                                     │
└─────────────────────────────────────────────────────────────┘
```
...
| Mobile nav drawer | Trong `site-header.tsx` |

---

### 1.3 Shell C — Dashboard (app)

Dùng cho mọi trang sau đăng nhập: dashboard, journal, audio, ai, journey, account, settings, mood-test.
...
│ 262px    │  ngày · title · subtitle · PlanBadge · avatar  │
│          │  · Logout                                      │
│ 7 nav    ├──────────────────────────────────────────────────┤
│ + Admin  │  dashboard-scroll-area → {page content}        │
│ (nếu có) │                                                  │
└──────────┴──────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────┐
│  MobileAppHeader (title, subtitle, PlanBadge, avatar)       │
├─────────────────────────────────────────────────────────────┤
│  mobile-app-content (scroll)                                │
├─────────────────────────────────────────────────────────────┤
│  MobileTabBar: Trang chủ | Nhật ký | Âm thanh | Lắng nghe | Thêm │
└─────────────────────────────────────────────────────────────┘
         ↓ (tap "Thêm")
┌─────────────────────────────────────────────────────────────┐
│  MobileMoreSheet (bottom sheet)                             │
│  Hành trình · Tài khoản · Cài đặt · Quản trị · Đăng xuất   │
└─────────────────────────────────────────────────────────────┘
```
...
**Sidebar nav (7 mục):** Trang chủ → Nhật ký → Âm thanh → Lắng nghe → Hành trình → Tài khoản → Cài đặt (+ Quản trị nếu admin).

---

### 1.4 Shell D — Auth minimal

Dùng cho `/login`, `/register`.

```
AuthMinimalHeader (logo link về /)
└── main.shell (lg: 2 cột)
    ├── [lg only] liquid-panel — copy thương hiệu
    └── hero-card — AuthForm
```

| File | `src/components/auth/auth-minimal-header.tsx`, `auth-form.tsx` |
...
DashboardShell
└── DashboardHome
    ├── MoodCheckInModal (auto nếu chưa check-in hôm nay)
