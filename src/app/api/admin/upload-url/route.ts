  ...
  try {
    ...
    if (error) {
      if (error.message?.includes("not found") || error.message?.includes("does not exist")) {
        ...
        return NextResponse.json({ signedUrl: d2.signedUrl, path: d2.path, publicUrl: urlData.publicUrl });
      }
      return NextResponse.json({ error: error.message }, { status: 500 });
    }

    const { data: urlData } = admin.storage.from(bucket).getPublicUrl(data.path);
    return NextResponse.json({ signedUrl: data.signedUrl, path: data.path, publicUrl: urlData.publicUrl });
  } catch (err) {
