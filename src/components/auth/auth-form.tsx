...
export function AuthForm({ mode, next = "/dashboard" }: { mode: AuthMode; next?: string }) {
  ...
  return (
    <form action={onSubmit} noValidate className="auth-form flex min-h-0 w-full flex-1 flex-col">
      <div className="auth-form-scroll flex-1 overflow-y-auto overscroll-contain px-4 pt-4 md:px-5 md:pt-5">
        ...
        {canOAuth ? (
          <div className="mt-6 flex flex-col gap-2">
            ...
            >
              {oauthLoading === "github" ? (
                <span className="h-4 w-4 animate-spin rounded-full border-2 border-[var(--border)] border-t-transparent" />
              ) : (
                <GithubIcon />
              )}
              {mode === "login" ? "Đăng nhập" : "Đăng ký"} với GitHub
            </button>
        ...
        <div className={`mt-2 grid gap-4 ${mode === "register" ? "md:grid-cols-2" : ""}`}>
          {mode === "register" ? (
