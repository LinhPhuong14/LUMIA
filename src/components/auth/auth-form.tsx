...
export function AuthForm({ mode, next = "/dashboard" }: { mode: AuthMode; next?: string }) {
  ...
  return (
    <form action={onSubmit} className="flex w-full flex-col gap-4 p-4 md:p-5">
      <div>
        ...
        <p className="mt-3 text-sm leading-6 text-muted">
          {mode === "login" ? "Hôm nay mình bắt đầu nhẹ nhàng thôi." : "Bạn có thể xóa dữ liệu cảm xúc và lịch sử trò chuyện bất cứ lúc nào."}
        </p>
      </div>
      ...
      {mode === "register" ? (
        <label className="flex flex-col gap-2 text-sm font-medium text-matcha-deep">
          ...
          <input
            name="name"
            className="rounded-[22px] border border-matcha-soft bg-white px-4 py-3 outline-none ring-matcha/20 transition focus:ring-4"
      ...
      <button type="submit" disabled={loading} className="button-primary mt-2 disabled:opacity-60">
