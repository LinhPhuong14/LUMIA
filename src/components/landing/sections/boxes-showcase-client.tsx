...
import type { LandingBoxCardData } from "@/lib/landing-plans";
...
const iconBySlug: Record<string, LucideIcon> = Object.fromEntries(
  landingBoxCards.map((box) => [box.slug, box.icon]),
);

export function BoxesShowcaseClient({ boxes }: { boxes: LandingBoxCardData[] }) {
  const cards: LandingBoxCard[] = boxes.map((box) => ({
    ...box,
    icon: iconBySlug[box.slug] ?? Package,
  }));
  ...
  return (
    <section id="goi-lumia" className="relative overflow-hidden py-20 md:py-28">
      ...
      <div className="landing-pricing-shell">
        ...
        <LandingPricingCarousel boxes={cards} className="md:hidden" />
      </div>
    </section>
  );
}
